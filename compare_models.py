...
if __name__ == '__main__':
    ...
    for m in models:
        ...
        if m.name == "Light GBM":
            try:
                if hasattr(m.model, 'feature_importances_'):
                    importance_values = m.model.feature_importances_
        ...
        # Tree-based models
        elif hasattr(m.model, 'feature_importances_'):
            importance_values = m.model.feature_importances_
            importance_df = pd.DataFrame({
                'Feature': feature_names,
                "Importance": importance_values
        ...
        # ARIMAX models
        elif hasattr(m.model, 'model_fit') and hasattr(m.model.model_fit, 'params'):
            params = m.model.model_fit.params
